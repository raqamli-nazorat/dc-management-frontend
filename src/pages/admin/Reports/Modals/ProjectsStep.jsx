...
const ProjectsStep = ({ selectedList = [], onConfirm, onClose }) => {
     ...
     const getProjects = async ({ search }) => {
          ...
          setLoading(true);
          try {
               const { data } = await axiosAPI.get("projects/", { params });
     ...
     const toggleAll = () => {
          if (selectedIds.length === Projects?.length) {
               ...
               setSelectedIds(Projects?.map(emp => emp.id));
          }
     };
                              ...
                              {Projects?.map((project) => {
                                   ...
                                   return (
                                        <div
                                             ...
                                             className={`flex items-center justify-between gap-3 p-3 rounded-[16px] border cursor-pointer transition-all ${selectedIds.includes(project.id)
                                                  ? "border-indigo-500 bg-indigo-50/30"
                                                  : "border-gray-100 bg-[#F8F9FD] hover:border-gray-300"
                                                  }`}>
                                             <div className="flex items-center gap-3 flex-1 min-w-0">
                                                  <div className={`w-5 h-5 rounded-md border flex items-center justify-center transition-all ${selectedIds.includes(project.id) ? "bg-indigo-600 border-indigo-600" : "bg-white border-gray-300"}`}>
                                                       {selectedIds.includes(project.id) && <IoCheckmark className="text-white" size={14} />}
                                                  </div>
                                             ...
                                             <span className="text-sm  whitespace-nowrap">{projectDate}</span>
                                        </div>
                                   );
                              })}
