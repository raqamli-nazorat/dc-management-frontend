...
function MonthYearPicker({ label, value, onChange }) {
  ...
  const [year, setYear] = useState(value ? value.slice(0, 4) : String(new Date().getFullYear()))

  const display = value
    ? `${MONTHS.find(m => m.value === value.slice(5, 7))?.label ?? ''} ${value.slice(0, 4)}`
    : ''
  ...
  return (
    <div ref={ref} className="relative">
      ...
      <button type="button" onClick={() => setOpen(o => !o)}
        ...
        <span className="flex-1 text-left truncate">{display || 'Oy tanlang'}</span>
      ...
      {open && (
        <div className="absolute top-full left-0 mt-1 z-60 w-full rounded-2xl shadow-xl border overflow-hidden
          ...
          <div className="flex items-center gap-1 px-3 py-2 border-b border-[#F1F3F9] dark:border-[#292A2A]">
            {YEARS.map(y => (
              <button key={y} type="button" onClick={() => setYear(y)}
                className={`flex-1 py-1.5 rounded-lg text-xs font-semibold cursor-pointer transition-colors
                  ${year === y ? 'bg-[#3F57B3] text-white' : 'text-[#5B6078] dark:text-[#C2C8E0] hover:bg-[#F1F3F9] dark:hover:bg-[#292A2A]'}`}>
                {y}
              </button>
            ))}
          </div>
          <div className="grid grid-cols-3">
            {MONTHS.map((m, i) => {
              const v = `${year}-${m.value}`
              const isSelected = value === v
              return (
                <button key={m.value} type="button" onClick={() => { onChange(v); setOpen(false) }}
                  className={`py-2.5 text-sm cursor-pointer transition-colors
                    ${i % 3 !== 2 ? 'border-r border-[#F1F3F9] dark:border-[#292A2A]' : ''}
                    ${i < 9 ? 'border-b border-[#F1F3F9] dark:border-[#292A2A]' : ''}
                    ${isSelected ? 'bg-[#EEF1FB] text-[#3F57B3] font-semibold dark:bg-[#292A2A] dark:text-[#7F95E6]' : 'text-[#1A1D2E] dark:text-[#FFFFFF] hover:bg-[#F8F9FC] dark:hover:bg-[#292A2A]'}`}>
                  {m.label}
                </button>
              )
            })}
...
function SalaryFilterModal({ onClose, onApply, initial }) {
  ...
  const IS_CONFIRMED_OPTIONS = [
    { label: 'Tasdiqlangan',   value: 'true' },
    { label: 'Tasdiqlanmagan', value: 'false' },
  ]
  const { open: cOpen, setOpen: setCOpen, ref: cRef } = useDropdown()
      ...
      <div className="relative w-full max-w-[600px] rounded-2xl shadow-2xl bg-white dark:bg-[#222323]">
        ...
        <div className="px-6 pb-4 flex flex-col gap-4">
          ...
          <div className="grid grid-cols-3 gap-3">
            <MonthYearPicker label="Oydan" value={f.month_gte} onChange={v => set('month_gte', v)} />
            <MonthYearPicker label="Oygacha" value={f.month_lte} onChange={v => set('month_lte', v)} />
            <div ref={cRef}>
              <label className={labelCls}>Holati</label>
              <div className="relative">
                <button type="button" onClick={() => setCOpen(o => !o)}
                  className={`w-full flex items-center justify-between px-3 py-2.5 rounded-xl text-sm border cursor-pointer
                    bg-white border-[#E2E6F2] dark:bg-[#191A1A] dark:border-[#292A2A]
                    ${f.is_confirmed ? 'text-[#1A1D2E] dark:text-[#FFFFFF]' : 'text-[#8F95A8] dark:text-[#C2C8E0]'}`}>
                  <span className="flex-1 text-left truncate">
                    {IS_CONFIRMED_OPTIONS.find(o => o.value === f.is_confirmed)?.label || 'Tanlang'}
                  </span>
                  <div className="flex items-center gap-1 shrink-0 ml-1">
                    {f.is_confirmed && (
                      <span onMouseDown={e => { e.stopPropagation(); set('is_confirmed', '') }}
                        className="text-[#B6BCCB] hover:text-[#5B6078] cursor-pointer"><FaXmark size={11} /></span>
                    )}
                    <FaChevronDown size={11} className={`text-[#8F95A8] transition-transform ${cOpen ? 'rotate-180' : ''}`} />
                  </div>
                </button>
                {cOpen && (
                  <div className="absolute top-full left-0 mt-1 z-60 w-full rounded-2xl shadow-xl border overflow-hidden
                    bg-white border-[#E2E6F2] dark:bg-[#222323] dark:border-[#292A2A]">
                    {IS_CONFIRMED_OPTIONS.map((o, i) => (
                      <button key={o.value} type="button" onClick={() => { set('is_confirmed', o.value); setCOpen(false) }}
                        className={`w-full text-left px-4 py-2.5 text-sm cursor-pointer
                          ${i < IS_CONFIRMED_OPTIONS.length - 1 ? 'border-b border-[#F1F3F9] dark:border-[#292A2A]' : ''}
                          ${f.is_confirmed === o.value ? 'bg-[#EEF1FB] text-[#3F57B3] font-semibold dark:bg-[#292A2A] dark:text-[#7F95E6]' : 'text-[#1A1D2E] dark:text-[#FFFFFF] hover:bg-[#F8F9FC] dark:hover:bg-[#292A2A]'}`}>
                        {o.label}
                      </button>
                    ))}
                  </div>
          ...
          <div>
            <div className="flex items-center justify-between mb-1.5">
              <label className={labelCls + ' mb-0'}>Jarima miqdori (UZS)</label>
              <Toggle checked={f.showFine} onChange={v => set('showFine', v)} />
            </div>
            {f.showFine && (
              <div className="grid grid-cols-2 gap-3">
                <input className={`${iCls} text-[#E02D2D] dark:text-[#FA5252]`} placeholder="dan: 0"
                  value={f.penalty_amount__gte} onChange={e => set('penalty_amount__gte', e.target.value.replace(/[^\d]/g, ''))} />
                <input className={`${iCls} text-[#E02D2D] dark:text-[#FA5252]`} placeholder="gacha: 0"
          ...
          <div className="px-6 pb-4 grid grid-cols-2 gap-3">
            ...
            <div>
              ...
              <div className="flex gap-2">
                <div className="w-16 shrink-0 h-[42px] px-3 py-2.5 rounded-xl text-sm text-center border flex items-center justify-center
                  bg-white border-[#E2E6F2] text-[#1A1D2E] dark:bg-[#191A1A] dark:border-[#292A2A] dark:text-[#FFFFFF]">
                  {u.passport_series?.slice(0, 2) ?? ''}
                </div>
                <div className="flex-1 h-[42px] px-3 py-2.5 rounded-xl text-sm border flex items-center
                  bg-white border-[#E2E6F2] text-[#1A1D2E] dark:bg-[#191A1A] dark:border-[#292A2A] dark:text-[#FFFFFF]">
                  {u.passport_series?.slice(2)?.trim() ?? ''}
                </div>
              </div>
            </div>
            <Field label="Oylik maosh (UZS)" value={fmt(user.fixed_salary)} />
            <Field label="KPI bonus (UZS)" value={fmt(user.kpi_bonus)} />
...
export default function SalaryPage() {
  ...
  const hasFilter = Object.entries(filters).some(([k, v]) => k !== 'showFine' && v && v !== false)
  ...
  const loadPayrolls = async (f = filters, q = search, pg = 1) => {
    ...
    try {
      ...
      if (f.month_gte)           params.month__gte          = f.month_gte
      if (f.month_lte)           params.month__lte          = f.month_lte
      if (f.is_confirmed !== '') params.is_confirmed        = f.is_confirmed
  ...
  const handleSearch = (val) => { setSearch(val); loadPayrolls(filters, val, 1) }
  const handleApplyFilter = (f) => { setFilters(f); setShowFilter(false); loadPayrolls(f, search, 1) }
      ...
      <div className="shrink-0 bg-[#F8F9FC] dark:bg-[#191A1A]">
        ...
        <div className="flex items-center justify-between mb-3">
          ...
          <div className="relative group flex items-center gap-2">
            <div className="absolute right-10 top-1/2 -translate-y-1/2 z-20 w-[220px] px-4 py-3 rounded-2xl shadow-xl text-[12px]
              text-[#1A1D2E] dark:text-[#FFFFFF] bg-white dark:bg-[#222323] border border-[#E2E6F2] dark:border-[#292A2A]
            ...
            <tbody>
              {data.map((u, idx) => (
                <tr key={u.id} onClick={() => handleRowClick(u)}
                  ...
                  <td className="px-4 py-3 text-center sticky right-0 bg-[#F8F9FC] dark:bg-[#191A1A] group-hover:bg-[#F0F1F5] dark:group-hover:bg-[#202221]"
