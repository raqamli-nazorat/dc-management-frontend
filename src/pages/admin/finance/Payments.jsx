...
export default function PaymentsPage() {
  ...
  const loadPayments = async (f = filters, q = search) => {
    setLoading(true)
    try {
      const data = await apiGetPayments(f, q)
      setPayments(data)
    } catch (err) {
      console.error(err)
  ...
  return (
    <div className="flex flex-col h-[calc(100vh-120px)]">
          ...
          <table className="w-full text-sm whitespace-nowrap">
            <thead className="sticky top-0 z-10 bg-[#F8F9FC] dark:bg-[#191A1A]">
              <tr className="border-b border-[#E2E6F2] dark:border-[#292A2A]">
                {selecting && <th className="w-10 px-4 py-3 text-left bg-[#F8F9FC] dark:bg-[#191A1A]"><input type="checkbox" checked={allSelected} onChange={toggleAll} className="cursor-pointer accent-[#3F57B3]" /></th>}
      ...
      {detailPayment && (
        <XarajatDetailModal payment={detailPayment}
