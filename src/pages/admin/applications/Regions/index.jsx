          ...
          <div className="flex items-center gap-5">
            <div className="relative">
              ...
              <input
                ...
                className="pl-8 pr-3 outline-none transition-colors bg-slate-100 border border-[var(--stroke-sub)] text-[var(--text-strong)] placeholder-[var(--text-soft)] dark:bg-[var(--bg-elevation-1)] dark:border-[var(--stroke-soft)] dark:text-[var(--text-strong)] dark:placeholder-[var(--text-sub)]"
                style={{ fontSize: 13, fontWeight: 500, padding: '6px 12px 6px 32px', borderRadius: 12 }}
              />
            </div>
          </div>

          <div className="overflow-y-auto h-[calc(100vh-210px)] rounded-xl">
              ...
              <tbody>
                {applications?.map((application, idx) => (
                  <tr
                    ...
                    className="transition-colors cursor-pointer border-b border-[var(--stroke-soft)] dark:border-[var(--stroke-soft)]"
                  >
                    ...
                    <td className='flex justify-end pr-6 py-3' onClick={(e) => e.stopPropagation()}>
                      <button
                        ...
                        className={`relative w-10 h-5 rounded-full transition-colors cursor-pointer ${application?.is_application ? 'bg-[var(--accent-strong)]' : 'bg-[var(--stroke-sub)] dark:bg-[var(--bg-elevation-2)]'}`}
                      >
                        <span
            ...
            {applications.length === 0 && (
              <div className="py-16 text-center text-sm text-[var(--text-disabled)] dark:text-[var(--text-soft)]">Viloyatlar topilmadi</div>
            )}
          </div>
        </div>
      </div>
