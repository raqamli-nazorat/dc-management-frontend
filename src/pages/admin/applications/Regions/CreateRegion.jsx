...
const CreatePosition = ({ onClose, refetch }) => {
     ...
     const handleCreate = async () => {
          try {
               await axiosAPI.post('applications/regions/', form)
     ...
     const inputCls = 'w-full px-3 py-2.5 rounded-lg text-sm outline-none border transition-colors bg-white text-[var(--text-strong)] placeholder-[var(--text-disabled)] dark:bg-[#191A1A] dark:text-[#FFFFFF] dark:placeholder-[#8E95B5] border-[var(--stroke-sub)] focus:border-[var(--accent-sub)] dark:border-[#292A2A]'
          ...
          <div className="fixed inset-0 z-50 flex items-center justify-center overflow-y-auto modal-scroll py-8 px-4">
               ...
               <div className="relative w-full max-w-[600px] rounded-2xl shadow-2xl bg-white dark:bg-[#222323]">
                    ...
                    <div className="px-7 pb-2 flex flex-col gap-4">
                         ...
                         <div className="flex justify-between items-center">
                              ...
                              <button
                                   ...
                                   className={`relative w-10 h-5 rounded-full transition-colors cursor-pointer ${form.is_application ? 'bg-[var(--accent-strong)]' : 'bg-[var(--stroke-sub)] dark:bg-[#292A2A]'}`}
                              >
                                   <span
                    ...
                    <div className="px-7 py-5 flex items-center justify-end gap-3">
                         <button onClick={onClose} className="flex items-center gap-2 px-5 py-2.5 rounded-lg text-sm font-medium transition-colors cursor-pointer text-[var(--text-sub)] hover:bg-[#F1F3F9] dark:text-[#C2C8E0] dark:hover:bg-[#292A2A]">
                              <FaXmark size={14} />
                              Yopish
                         </button>
                         <button
                              onClick={handleCreate}
                              className="flex items-center gap-2 px-5 py-2.5 rounded-lg text-sm font-semibold transition-colors cursor-pointer bg-[var(--accent-strong)] text-white hover:bg-[var(--accent-sub)]"
                         >
                              <MdCheck size={16} />
                              Qo'shish
                         </button>
