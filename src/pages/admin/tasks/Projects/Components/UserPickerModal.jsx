...
import { FaArrowLeft, FaCheck, FaXmark, FaChevronDown } from "react-icons/fa6"
import { axiosAPI } from "../../../../../service/axiosAPI"

const labelCls = 'block text-xs font-medium text-[var(--text-sub)] dark:text-[var(--text-sub)] mb-1.5'

export const UserPickerModal = ({ title, selected, onConfirm, onClose, roles = "employee" }) => {
    ...
    const fetchUsers = async (searchTerm = '') => {
        setLoading(true)
        try {
            const { data } = await axiosAPI.get("users/all/", { params: { search: searchTerm, roles } })
            setUsersList(data?.data?.results || data?.results || [])
        } catch (error) {
            console.error(error)
        } finally {
            setLoading(false)
    ...
    const toggleAll = () => {
        if (temp.length === usersList.length) setTemp([])
        else setTemp(usersList.map(u => u.id))
    }

    const handleConfirm = () => {
        onConfirm(usersList.filter(u => temp.includes(u.id)))
    }
                ...
                <div className="flex-1 overflow-y-auto px-4 pb-2 flex flex-col gap-2">
                    {loading ? (
                        <div className="flex justify-center py-8">
                            <div className="animate-spin rounded-full h-6 w-6 border-b-2 border-[var(--accent-strong)]" />
                        </div>
                    ) : usersList.length === 0 ? (
                                ...
                                >
                                    ...
                                    <div className="min-w-0 flex-1">
                                        ...
                                        <p className="text-xs text-[var(--text-soft)] dark:text-[var(--text-sub)] truncate">
                                            {u?.position_info?.name || u?.position || '—'}
                                        </p>
                                    </div>
                                </button>
                            )
