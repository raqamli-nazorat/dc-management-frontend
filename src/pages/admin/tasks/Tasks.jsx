...
export default function TasksPage() {
  ...
  const handleDuplicate = async (id) => {
    ...
    try {
      ...
      // Fayllar ro'yxati — javob shakli har xil bo'lishi mumkin
      let attachments = []
      try {
        const { data } = await axiosAPI.get(`task-attachments/?task=${id}`)
        ...
        // Fayllar olinmasa ham vazifani takrorlashga to'sqinlik qilmaymiz
        toast.error('Diqqat', "Vazifa fayllari yuklanmadi, ularni qo'lda qo'shishingiz kerak")
      }

      setDuplicateTaskData({ ...task, attachments })
      setShowAdd(true)
    } catch (err) {
      toast.error('Xatolik', "Vazifa ma'lumotlarini yuklashda xatolik")
    } finally {
